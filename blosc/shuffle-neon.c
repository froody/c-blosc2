...
static void shuffle2_neon(
    uint8_t* const dest, const uint8_t* const src, const size_t vectorizable_elements, const size_t total_elements) {
  size_t i, k;
...
/* Routine optimized for shuffling a buffer for a type size of 12 bytes. */
static void shuffle12_neon(
    uint8_t* const dest, const uint8_t* const src, const size_t vectorizable_elements, const size_t total_elements) {
  size_t i, k;
  static const size_t bytesoftype = 12;
  uint8x8x2_t r0[8];
  uint16x4x2_t r1[8];
  uint32x2x2_t r2[8];

  for (i = 0, k = 0; i < vectorizable_elements * bytesoftype; i += 96, k++) {
    /* Load and interleave groups of 8 bytes (96 bytes total) to the structure r0 */
    uint32x2_t zero = vdup_n_u32(0);
    uint8x8_t e0_8 = vreinterpret_u8_u32(vld1_lane_u32((const uint32_t*)(src + i + 0 * 12 + 8), zero, 0));
    uint8x8_t e1_8 = vreinterpret_u8_u32(vld1_lane_u32((const uint32_t*)(src + i + 1 * 12 + 8), zero, 0));
    uint8x8_t e2_8 = vreinterpret_u8_u32(vld1_lane_u32((const uint32_t*)(src + i + 2 * 12 + 8), zero, 0));
    uint8x8_t e3_8 = vreinterpret_u8_u32(vld1_lane_u32((const uint32_t*)(src + i + 3 * 12 + 8), zero, 0));
    uint8x8_t e4_8 = vreinterpret_u8_u32(vld1_lane_u32((const uint32_t*)(src + i + 4 * 12 + 8), zero, 0));
    uint8x8_t e5_8 = vreinterpret_u8_u32(vld1_lane_u32((const uint32_t*)(src + i + 5 * 12 + 8), zero, 0));
    uint8x8_t e6_8 = vreinterpret_u8_u32(vld1_lane_u32((const uint32_t*)(src + i + 6 * 12 + 8), zero, 0));
    uint8x8_t e7_8 = vreinterpret_u8_u32(vld1_lane_u32((const uint32_t*)(src + i + 7 * 12 + 8), zero, 0));

    r0[0] = vzip_u8(vld1_u8(src + i + 0 * 12), vld1_u8(src + i + 1 * 12));
    r0[1] = vzip_u8(e0_8, e1_8);
    r0[2] = vzip_u8(vld1_u8(src + i + 2 * 12), vld1_u8(src + i + 3 * 12));
    r0[3] = vzip_u8(e2_8, e3_8);
    r0[4] = vzip_u8(vld1_u8(src + i + 4 * 12), vld1_u8(src + i + 5 * 12));
    r0[5] = vzip_u8(e4_8, e5_8);
    r0[6] = vzip_u8(vld1_u8(src + i + 6 * 12), vld1_u8(src + i + 7 * 12));
    r0[7] = vzip_u8(e6_8, e7_8);
...
static void unshuffle12_neon(
  ...
  for (i = 0, k = 0; i < vectorizable_elements * bytesoftype; i += 96, k++) {
    /* Load and interleave groups of 8 bytes (96 bytes) to the structure r0 */
    ...
    r0[6] = vzip_u8(zero, zero);
    r0[7] = vzip_u8(zero, zero);
    ...
    /* Store the results in the destination vector */
    vst1_u8(dest + i + 0 * 12, vreinterpret_u8_u32(r2[0].val[0]));
    vst1_lane_u32((uint32_t*)(dest + i + 0 * 12 + 8), r2[4].val[0], 0);

    vst1_u8(dest + i + 1 * 12, vreinterpret_u8_u32(r2[0].val[1]));
    vst1_lane_u32((uint32_t*)(dest + i + 1 * 12 + 8), r2[4].val[1], 0);

    vst1_u8(dest + i + 2 * 12, vreinterpret_u8_u32(r2[1].val[0]));
    vst1_lane_u32((uint32_t*)(dest + i + 2 * 12 + 8), r2[5].val[0], 0);

    vst1_u8(dest + i + 3 * 12, vreinterpret_u8_u32(r2[1].val[1]));
    vst1_lane_u32((uint32_t*)(dest + i + 3 * 12 + 8), r2[5].val[1], 0);

    vst1_u8(dest + i + 4 * 12, vreinterpret_u8_u32(r2[2].val[0]));
    vst1_lane_u32((uint32_t*)(dest + i + 4 * 12 + 8), r2[6].val[0], 0);

    vst1_u8(dest + i + 5 * 12, vreinterpret_u8_u32(r2[2].val[1]));
    vst1_lane_u32((uint32_t*)(dest + i + 5 * 12 + 8), r2[6].val[1], 0);

    vst1_u8(dest + i + 6 * 12, vreinterpret_u8_u32(r2[3].val[0]));
    vst1_lane_u32((uint32_t*)(dest + i + 6 * 12 + 8), r2[7].val[0], 0);

    vst1_u8(dest + i + 7 * 12, vreinterpret_u8_u32(r2[3].val[1]));
    vst1_lane_u32((uint32_t*)(dest + i + 7 * 12 + 8), r2[7].val[1], 0);
  }
}

/* Routine optimized for unshuffling a buffer for a type size of 16 bytes. */
static void unshuffle16_neon(
    uint8_t* const dest, const uint8_t* const src, const size_t vectorizable_elements, const size_t total_elements) {
  size_t i, k;
  static const size_t bytesoftype = 16;
  uint8x8x2_t r0[8];
...
void shuffle_neon(const int32_t bytesoftype, const int32_t blocksize, const uint8_t* const _src, uint8_t* _dest) {
  int32_t vectorized_chunk_size = 1;
  if (bytesoftype == 2 || bytesoftype == 4) {
    vectorized_chunk_size = bytesoftype * 16;
  } else if (bytesoftype == 8 || bytesoftype == 12 || bytesoftype == 16) {
  ...
  case 8:
    ...
    break;
  case 12:
    shuffle12_neon(_dest, _src, vectorizable_elements, total_elements);
    break;
  case 16:
...
void unshuffle_neon(const int32_t bytesoftype, const int32_t blocksize, const uint8_t* const _src, uint8_t* _dest) {
  int32_t vectorized_chunk_size = 1;
  if (bytesoftype == 2 || bytesoftype == 4) {
    vectorized_chunk_size = bytesoftype * 16;
  } else if (bytesoftype == 8 || bytesoftype == 12 || bytesoftype == 16) {
  ...
  case 8:
    ...
    break;
  case 12:
    unshuffle12_neon(_dest, _src, vectorizable_elements, total_elements);
    break;
  case 16:
...
void shuffle_neon(const int32_t bytesoftype, const int32_t blocksize, const uint8_t* const _src, uint8_t* const _dest) {
  abort();
}

void unshuffle_neon(const int32_t bytesoftype, const int32_t blocksize, const uint8_t* _src, uint8_t* _dest) {
  abort();
}
